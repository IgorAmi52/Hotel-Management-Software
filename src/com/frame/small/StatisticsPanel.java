...
public class StatisticsPanel extends JPanel implements Panel{
	...
	public StatisticsPanel() {
		...
		chart = new XYChartBuilder().width(800).height(600).title("Monthly Profit by Room Type").xAxisTitle("Month").yAxisTitle("Profit").build();
        ...
        cleanersChart = new PieChartBuilder().width(800).height(600).title("Obstruction of maids for the last 30 days").build();
        ...
        // Create Chart
        reservationChart = new PieChartBuilder().width(800).height(600).title("Status of reservations in last 30 days").build();
	...
	public void reset() {
		...
		clearChartData(reservationChart);
		 try {
				yearlyRoomTypeMap = ReportsService.getYearlyRoomTypeStatistics(2024);
